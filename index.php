...
<body>
    <div id="qingciSplash" aria-hidden="true">
        <div class="qingciSplashInner">
            <div class="qingciSplashLy">LY数据号</div>
            <div class="qingciSplashQc">清辞内核</div>
        </div>
    </div>
    <style>
        #qingciSplash{position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;background:#031b12;opacity:1;pointer-events:auto;transition:opacity .8s ease}
        #qingciSplash .qingciSplashInner{text-align:center;transform:scale(.58);opacity:0;animation:qingciSplashIn 1.15s cubic-bezier(.22,1,.36,1) forwards}
        #qingciSplash .qingciSplashLy,#qingciSplash .qingciSplashQc{font-family:Arial,"Microsoft YaHei",sans-serif;font-weight:700;line-height:1.15;letter-spacing:4px;text-shadow:0 0 18px rgba(105,255,139,.16)}
        #qingciSplash .qingciSplashLy{font-size:clamp(30px,5vw,52px);color:#075c32}
        #qingciSplash .qingciSplashQc{margin-top:12px;font-size:clamp(24px,4vw,42px);color:#7dff9a;letter-spacing:5px}
        #qingciSplash.qingciSplashOut{opacity:0;pointer-events:none}
        @keyframes qingciSplashIn{0%{opacity:0;transform:scale(.58)}65%{opacity:1;transform:scale(1.06)}100%{opacity:1;transform:scale(1)}}
    </style>
    <script>
    (function(){var s=document.getElementById('qingciSplash');if(!s)return;setTimeout(function(){s.classList.add('qingciSplashOut');setTimeout(function(){if(s.parentNode)s.parentNode.removeChild(s)},850)},1500);})();
    </script>
    <!-- 页面背景双图层 -->
    <div class="bg-stage">
